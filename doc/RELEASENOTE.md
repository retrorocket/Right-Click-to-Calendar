...
- Chromium向けアップデート：トークンの保存期限をブラウザが閉じるまでに変更しました。

### 2024/06/20: v3.68

- Chrome, Chromiumをアップデートするとイベントの登録ができなくなるバグを修正
...
- 詳細設定の内容を正規表現設定のデフォルト値でも使用するように変更

### 2023/12/30: v3.65

- 予定の書いてあるURLを「予定の詳細」にセットするオプションを追加
- iframe内で選択したテキストが予定にセットできないバグを修正

### 2023/12/29: v3.61

- Chromium向けアップデート：トークンの取得方法を変更。内部的な変更なため、アプリの動作に変更はありません。

### 2023/12/26: v3.60

- どうしてもGoogle Calendarと連携できないとき用のオプションを追加（ベータ機能）
  - Googleのドキュメントに存在しないAPIを使用しているため、突然動かなくなる、もしくは、機能を削除する可能性があります。積極的にサポートする予定もありません。

### 2023/09/06: v3.54

- 内部で使用しているライブラリのバージョンアップ
