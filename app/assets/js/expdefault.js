...
export const expDefault = (stext) => {
  ...
  let enMatch = [...stext.matchAll(new RegExp(String.raw`\b${monthsPattern}\.? (?<day>\d{1,2})(?:[a-z]{2}){0,1}\b(?:,? ?(?<year>\d{4}))?`, "g"))].map(match => match.groups);
  if (enMatch.length < 1) {
    enMatch = [...stext.matchAll(new RegExp(String.raw`\b(?<day>\d{1,2})(?:[a-z]{2}){0,1} ${monthsPattern}\.?(?:,? ?(?<year>\d{4}))?`, "g"))].map(match => match.groups);
  }
  if (enMatch.length > 0) {
    const em0 = enMatch[0];
    smon = em0.mon ? months.indexOf(em0.mon.substring(0, 3)) + 1 : smon;
    sday = em0.day ? em0.day : sday;
    syear = em0.year ? em0.year : syear;
    if (enMatch.length > 1) {
      const em1 = enMatch[1];
      emon = em1.mon ? months.indexOf(em1.mon.substring(0, 3)) + 1 : emon;
